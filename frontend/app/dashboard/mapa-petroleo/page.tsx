...
function calcularBPDC(registros: any[]): number {
...
export default function MapaPetroleoPage() {
  ...
  const kpis = useMemo(() => {
    ...
    return { bpdc, camposSet: campos, operadorasSet, municipiosSet: municipios };
  }, [filtrados]);
  ...
  const mapData = useMemo(() => {
    ...
    Object.entries(porMuni).forEach(([municipio, regs]) => {
      ...
      const bpdcPromedio = calcularBPDC(regs);
      ...
      if (bpdcPromedio > 0) {
        points.push({
          ...
          weight: Math.round(bpdcPromedio), // BPDC promedio del municipio
          label: municipio,
  ...
  if (isLoading) return <Loading message="Cargando mapa de producción de petróleo..." />;
  if (error) return <div>Error cargando el mapa: {String(error)}</div>;

  return (
    <div className="page-content" style={{ display: 'flex', flexDirection: 'column' }}>
      <div className="page-header" style={{ flexShrink: 0 }}>
        <h1 style={{ color: 'var(--color-primary)', fontWeight: 900, display: 'flex', alignItems: 'center', gap: 12 }}>
          <Droplets size={32} strokeWidth={2.5} style={{ color: "var(--color-primary)" }} /> 
          Producción Petróleo: Perspectiva Geográfica
        </h1>
        <p>Evolución de producción (BPDC) en territorio ({filtrados.length.toLocaleString('es-CO')} registros mapeados {filtroAnio !== 'Todos' ? `en ${filtroAnio}` : ''})</p>
      </div>
      ...
      <div style={{ height: 'calc(100vh - 350px)', minHeight: '500px', position: 'relative', borderRadius: 16, overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', border: '1px solid var(--color-border)' }}>
        {/* Aplicamos un color range distinto para petróleo (tonos cálidos: amarillos, naranjas, rojos) */}
        <PremiumMap 
          data={mapData} 
          layerType={layerType} 
          colorRange={[
          ...
          customTooltip={(object, allData) => {
            ...
            if (object.points && object.points.length > 0) {
              ...
              // Si Deck.gl está usando GPU Aggregation, no expone points ni pointIndices.
              // Solo nos da object.position (el centro geométrico del hexágono).
              // Buscamos espacialmente todos los datos que caen en un radio de ~15km (0.15 grados).
              ...
              // Fallback de seguridad para no romper el cálculo del BPDC
            ...
            // Extraer el BPDC ya calculado por el HexagonLayer
            const totalBPDC = object.elevationValue ?? object.colorValue ?? pointsInBin.reduce((sum: number, p: any) => sum + (p.weight || 0), 0);
            ...
            return {
              html: `
                ...
                <div style="display: flex; gap: 8px; align-items: center; padding-top: 8px; border-top: 1px dashed rgba(255,255,255,0.2);">
                  <span style="font-weight:bold; font-size: 14px; color: #cbd5e1;">BPDC:</span> 
                  <span style="font-weight:900; color: #4ade80; font-size: 16px;">${Math.round(totalBPDC).toLocaleString('es-CO')}</span>
                </div>
              `,
              style: {
