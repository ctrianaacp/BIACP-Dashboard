      ...
      <div style={{ height: 'calc(100vh - 350px)', minHeight: '500px', position: 'relative', borderRadius: 16, overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', border: '1px solid var(--color-border)' }}>
        ...
        <PremiumMap 
          ...
          customTooltip={(object, allData) => {
            ...
            const totalBPDC = object.elevationValue ?? object.colorValue ?? pointsInBin.reduce((sum: number, p: any) => sum + ((p.source || p).weight || 0), 0);

            const debugStr = JSON.stringify({ 
              hasPoints: !!object.points, 
              pointsLen: object.points?.length, 
              hasIndices: !!object.pointIndices, 
              binLen: pointsInBin.length,
              firstBin: pointsInBin.length ? Object.keys(pointsInBin[0]) : null,
              fallback: pointsInBin[0] === object
            });
            ...
            return {
              html: `
                ...
                <div style="display: flex; gap: 5px; align-items: center; padding-top: 5px; border-top: 1px dashed rgba(255,255,255,0.2);">
                  <span style="font-weight:bold; font-size: 11px; color: #cbd5e1;">BPDC:</span> 
                  <span style="font-weight:900; color: #4ade80; font-size: 12px;">${Math.round(totalBPDC).toLocaleString('es-CO')}</span>
                </div>
              `,
              style: {
